...
public class PushNotificationManager {
	...
	/*
	 * To circumvent an issue with invalid server certificates, set to true to
	 * use a trust manager that will always accept server certificates,
	...
	public void initializeConnection(AppleNotificationServer server)
		...
		try {
			this.connectionToAppleServer = new ConnectionToNotificationServer(
					server);
			...
			logger.debug("Initialized Connection to Host: ["
					+ server.getNotificationServerHost() + "] Port: ["
					+ server.getNotificationServerPort() + "]: " + socket);
			...
			throw new CommunicationException(
					"Error creating connection with Apple server", e);
		}
	}
	...
	private String getCertificateChainDescription() {
		...
		try {
			...
			for (X509Certificate certificate : session
	...
	/**
	 * Initialize a connection using server settings from the previous
	 * connection.
	 * 
	 * @throws CommunicationException
	 *             thrown if a communication error occurs
	 ...
	 *             thrown if there is a problem with your keystore
	 */
	public void initializePreviousConnection() throws CommunicationException,
			KeystoreException {
		initializeConnection((AppleNotificationServer) this.connectionToAppleServer
				.getServer());
	}
	 ...
	 *             thrown if there is a problem with your keystore
	 */
	private void restartPreviousConnection() throws CommunicationException,
			KeystoreException {
		try {
	 ...
	 *             thrown if there is a problem with your keystore
	 */
	private int processedFailedNotifications() throws CommunicationException,
			KeystoreException {
		if (useEnhancedNotificationFormat) {
			...
			while (responsesReceived > 0) {
				...
				int remaining = responsesReceived = ResponsePacketReader
						.processResponses(this);
				if (remaining == 0) {
	 ...
	 * @param payload
	 *            the payload to send
	 * @return a pushed notification with details on transmission result and
	 *         error (if any)
	 * @throws CommunicationException
	 *             thrown if a communication error occurs
	 */
	public PushedNotification sendNotification(Device device, Payload payload)
			throws CommunicationException {
		return sendNotification(device, payload, true);
	}

	/**
	 * Send a notification to a multiple devices in a single connection and
	...
	public PushedNotifications sendNotifications(Payload payload,
			List<Device> devices) throws CommunicationException,
			KeystoreException {
		PushedNotifications notifications = new PushedNotifications();
		for (Device device : devices)
			notifications.add(sendNotification(device, payload, false,
	 ...
	 */
	public PushedNotifications sendNotifications(Payload payload,
			Device... devices) throws CommunicationException, KeystoreException {
		PushedNotifications notifications = new PushedNotifications();
		for (Device device : devices)
			notifications.add(sendNotification(device, payload, false,
					SEQUENTIAL_IDENTIFIER));
	 ...
	 *            the payload to send
	 * @param closeAfter
	 *            indicates if the connection should be closed after the payload
	 *            has been sent
	 * @return a pushed notification with details on transmission result and
	 *         error (if any)
	 * @throws CommunicationException
	 ...
	 */
	public PushedNotification sendNotification(Device device, Payload payload,
			boolean closeAfter) throws CommunicationException {
		return sendNotification(device, payload, closeAfter,
				SEQUENTIAL_IDENTIFIER);
	}
	 ...
	 *            the payload to send
	 * @param identifier
	 *            a unique identifier which will match any error reported later
	 *            (if any)
	 * @return a pushed notification with details on transmission result and
	 *         error (if any)
	 * @throws CommunicationException
	 ...
	 */
	public PushedNotification sendNotification(Device device, Payload payload,
			boolean closeAfter, int identifier) throws CommunicationException {
	 ...
	 *             thrown if a communication error occurs
	 */
	private void sendNotification(PushedNotification notification,
			boolean closeAfter) throws CommunicationException {
		try {
			...
			// revalidate it in case we were passed another implementation of
			// Device
			BasicDevice.validateTokenFormat(token);
			// PushedNotification pushedNotification = new
			// PushedNotification(device, payload);
			...
			{
				pushedNotifications.put(notification.getIdentifier(),
						notification);
			}
				...
				try {
					...
					try {
						if (!simulationMode) {
							...
							logger.debug("* Simulation only: would have streamed "
									+ bytes.length + "-bytes message now..");
						}
					} catch (Exception e) {
						if (e != null) {
					...
					if (streamConfirmed)
						logger.debug("At this point, the entire "
								+ bytes.length
								+ "-bytes message has been streamed out successfully through the SSL connection");

					success = true;
					logger.debug("Notification sent on "
							+ notification.getLatestTransmissionAttempt());
					notification.setTransmissionCompleted(true);

				} catch (IOException e) {
					// throw exception if we surpassed the valid number of retry
					// attempts
					if (notification.getTransmissionAttempts() >= retryAttempts) {
						...
						// Removing notification from map, since it is failed upto maximum retry counts.
						pushedNotifications.remove(notification.getIdentifier());
						
						throw e;

					} else {
						logger.info("Attempt failed (" + e.getMessage()
								+ ")... trying again");
						// Try again
						try {
							this.socket.close();
	 ...
	 */
	@Deprecated
	public void addDevice(String id, String token)
			throws DuplicateDeviceException, NullIdException,
	 ...
	 */
	@Deprecated
	public Device getDevice(String id) throws UnknownDeviceException,
			NullIdException {
	 ...
	 */
	@Deprecated
	public void removeDevice(String id) throws UnknownDeviceException,
			NullIdException {
	...
	// proxy settings.
	// * Use AppleNotificationServer.setProxy(..) to set a proxy for JavaPNS
	// only.
	// */
	 ...
	 * notification message is COMMAND | TOKENLENGTH | DEVICETOKEN |
	 * PAYLOADLENGTH | PAYLOAD or enhanced notification format: COMMAND |
	 * !Identifier! | !Expiry! | TOKENLENGTH| DEVICETOKEN | PAYLOADLENGTH |
	 * PAYLOAD See page 30 of Apple Push Notification Service Programming Guide
	 * 
	 * @param deviceToken
	 *            the deviceToken
	 * @param payload
	 ...
	 * @throws IOException
	 */
	private byte[] getMessage(String deviceToken, Payload payload,
			int identifier, PushedNotification message) throws IOException,
			Exception {
		...
		if (useEnhancedNotificationFormat) {
			// 4 bytes identifier (which will match any error packet received
			// later on)
			...
			if (requestedExpiry <= 0) {
				...
				long ttl = requestedExpiry * 1000; // time-to-live in
													// milliseconds
		...
		logger.debug("Built raw message ID " + identifier + " of total length "
				+ bytes.length);
		return bytes;
	}
	...
	/**
	 * Sets the DeviceFactory used by this PushNotificationManager. Usually
	...
	/**
	 * Set whether or not to enable the "trust all server certificates" feature
	 * to simplify SSL communications.
	 * 
	 ...
	 * to be written to local files. The last raw APSN message will be written
	 * to a "apns-message.bytes" file in the working directory. A detailed
	...
	private void preconfigurePayload(Payload payload, int identifier,
			String deviceToken) {
		try {
			...
			if (payload instanceof PushNotificationPayload) {
				...
				if (config == 1) {
					...
					pnpayload.addAlert(buildDebugAlert(payload, identifier,
							deviceToken));
				}
			}
		} catch (Exception e) {
	...
	private String buildDebugAlert(Payload payload, int identifier,
			String deviceToken) {
		StringBuilder alert = new StringBuilder();
		alert.append("JAVAPNS DEBUG ALERT " + (TESTS_SERIAL_NUMBER++) + "\n");

		/* Current date & time */
		alert.append(new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(System
				.currentTimeMillis()) + "\n");
		...
		alert.append(""
				+ deviceToken.substring(0, l)
				+ "�"
				+ deviceToken.substring(64 - l, 64)
				+ (useEnhancedNotificationFormat ? " [Id:"
						+ identifier
						+ "] "
						+ (payload.getExpiry() <= 0 ? "No-store" : "Exp:T+"
								+ payload.getExpiry()) : "") + "\n");

		/* Format & encoding */
		alert.append((useEnhancedNotificationFormat ? "Enhanced" : "Simple")
				+ " format / " + payload.getCharacterEncoding() + "" + "");
